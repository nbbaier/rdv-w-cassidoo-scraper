...
const H2_CLOSE_TAG = /<\/h2>/i;
const QUESTION_AFTER_H2 = /^([\s\S]*?)(?:Example|<hr|$)/i;
...
function findInterviewSection(document: Document): Element | undefined {
  return Array.from(document.querySelectorAll(".hr-section")).find((e) =>
    e.querySelector("h2")?.textContent?.toLowerCase().includes("interview")
  );
}

function isAnswersOnly(interviewSection: Element): boolean {
  ...
  return !hasQuestionStrong;
}

function findQuestionStrong(interviewSection: Element): Element | null {
  let questionStrong: Element | null = null;
  let maxLen = 0;
  for (const s of interviewSection.querySelectorAll("strong")) {
  ...
  return questionStrong;
}

function extractPlainTextQuestion(interviewSection: Element): string | null {
  const h2 = interviewSection.querySelector("h2");
  if (!h2) {
    return null;
  }
  const afterH2 = interviewSection.innerHTML.split(H2_CLOSE_TAG)[1];
  if (!afterH2) {
    return null;
  }
  const questionMatch = afterH2.match(QUESTION_AFTER_H2);
...
async function renderMarkdown(
  ...
  const wrapped = `<!doctype html><html><body>${innerHtml}</body></html>`;
  const result = await Defuddle(wrapped, sourceUrl, { markdown: true });
...
function buildQuestionDiv(
  document: Document,
  interviewSection: Element,
  questionStrong: Element
): HTMLDivElement {
  ...
  appendFollowingSiblings(questionDiv, walkAnchor, interviewSection);
  normalizeBrsToParagraphs(questionDiv);
  return questionDiv;
}

function appendFollowingSiblings(
  questionDiv: Element,
  walkAnchor: Element,
  interviewSection: Element
): void {
...
export async function extractQuestion(
  ...
  const interviewSection = findInterviewSection(document);
  if (!interviewSection) {
    return { status: "failed", reason: "no interview section" };
  }

  if (isAnswersOnly(interviewSection)) {
    return { status: "skipped", reason: "answers-only" };
  }

  const questionStrong = findQuestionStrong(interviewSection);

  if (!questionStrong) {
    const plainText = extractPlainTextQuestion(interviewSection);
    if (plainText) {
      const innerHtml = `<h2>Interview question of the week</h2><p>${plainText}</p>`;
      return {
        status: "ok",
        markdown: await renderMarkdown(innerHtml, sourceUrl),
      };
    }
    return { status: "failed", reason: "no question found" };
  }

  const questionDiv = buildQuestionDiv(
    document,
    interviewSection,
    questionStrong
  );
  return {
    status: "ok",
    markdown: await renderMarkdown(questionDiv.innerHTML, sourceUrl),
  };
}
